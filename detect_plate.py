...
if __name__ == '__main__':
    ...
    parser.add_argument('--height', type=int, default=1080, help='camera height')  # camera height
    parser.add_argument('--output', type=str, default='result', help='source')               #图片结果保存的位置
    #parser.add_argument('--video', type=int, default='', help='source')                       #视频的路径
    ...
    if not opt.video:     #处理图片
        ...
        #capture = cv2.VideoCapture(gstreamer_pipeline(sensor_id=video_name, capture_width=opt.width, capture_height=opt.height), cv2.CAP_GSTREAMER)
        fourcc = cv2.VideoWriter_fourcc('M','P', '4', 'V') 
        fps = capture.get(cv2.CAP_PROP_FPS)  # 帧数
        w, h = int(capture.get(cv2.CAP_PROP_FRAME_WIDTH)), int(capture.get(cv2.CAP_PROP_FRAME_HEIGHT))  # 宽高
        out = cv2.VideoWriter('result.mp4', fourcc, fps, (w, h))  # 写入视频
        frame_count = 0
        fps_all=0
        rate,FrameNumber,duration=get_second(capture)
        print(w, h, fps, rate, FrameNumber, duration)
        if capture.isOpened():
            while True:
                t1 = cv2.getTickCount()
                frame_count+=1
                ...
                fps=1.0/infer_time
                fps_all+=fps
                str_fps = f'fps:{fps:.3f}'
                
                cv2.putText(ori_img,str_fps,(20,30),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),2)
                cv2.imshow("plate",ori_img)
                cv2.waitKey(1)
                out.write(ori_img)
